...
def generate_launch_description():
    ...
    config_file_path_1st = os.path.join(
        get_package_share_directory("scl_amr"), "config", "hokuyo_ether.yaml"
    )

    # パラメータファイルのパス設定（2台目）
    config_file_path_2nd = os.path.join(
        get_package_share_directory("scl_amr"), "config", "hokuyo_ether_2nd.yaml"
    )
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument("node_name_1st", default_value="urg_back"),
            DeclareLaunchArgument("node_name_2nd", default_value="urg_front"),
            DeclareLaunchArgument("scan_topic_name_1st", default_value="scan_back"),
            DeclareLaunchArgument("scan_topic_name_2nd", default_value="scan_front"),
            lifecycle_node_1st,
            lifecycle_node_2nd,
