...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("UI界面 without RViz")

        self.central_widget = QWidget(self)
        self.setCentralWidget(self.central_widget)

        # Create main layout (VBoxLayout for the entire window)
        main_layout = QVBoxLayout(self.central_widget)

        # Create a widget for right side controls (buttons and lineEdits)
        right_widget = QWidget()
        right_layout = QVBoxLayout(right_widget)
        main_layout.addWidget(right_widget, alignment=Qt.AlignRight)  # Align the layout to the right

        # Add lineEdit for 信錦工廠 (on top)
        ...
        self.create_line_edit(right_layout, "lineEdit_4", "3  選擇任務", "small", True)
